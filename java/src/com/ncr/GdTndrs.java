...
public class GdTndrs extends Action {
    ...
    static int tnd_print() {
        ...
        logger.info("tnd[itm.tnd].eftPlugin >" + tnd[itm.tnd].eftPlugin + "<");
        eftPluginManager.writeInfo(tnd[itm.tnd].eftPlugin, tra, itm);
        ...
        if (itm.number.length() > 0) {
            ...
            } else if (ECommerce.getTransactionResumed().length() != 0 && itm.text.startsWith("RECEIPT#")) {
        ...
        if (tra.mode == M_GROSS) {
            tra.print = ReceiptPrintManager.getInstance().ask(Mnemo.getMenu(114));
        }
        tnd_symbol();
        tra.bal = 0 - tra.bal;
        if (itm.tnd != 1) {
    ...
    public int action1(int spec) {
        ...
        if ((tra.spf1 & M_TRVOID) > 0) {
            ...
            itm.flag = T_ONSLIP;
        // if ((ptr.flag & T_NUMBER) > 0) { // TSC-MOD2014-AMZ#DEL
        if (!GdTsc.isAuthCodeEnabled(ptr.type) && (ptr.flag & T_NUMBER) > 0) { // TSC-MOD2014-AMZ#ADD
            if (ptr.type == 'B')
    ...
    public int action2(int spec) {
        ...
        switch (spec) {
            ...
            default:
                ...
                // In case of Ecommerce transaction forcing Tender type A
                if (ECommerceManager.getInstance().isEnabled() && ECommerceManager.getInstance().isTransactionStarted())
                    ptr.type = 'A';
                // TSC-MOD2014-AMZ#END
                itm.number = input.pb;
                if (ptr.type == 'D') {
        ...
        if (ECommerce.getTenderProperties().keySet().contains(itm.tnd)) {
            tra.cleanPo = true;
            tra.special = Transact.ECOMMERCE;
    ...
    public int action3(int spec) {
        ...
        // TSC-MOD2014-AMZ#BEG
        if (GdTsc.isAuthCodeEnabled(ptr.type) && (ptr.flag & T_NUMBER) > 0) {
        ...
        if (tnd[itm.tnd].eftPlugin.equals("M")) {
            logger.info("It's a verifone tender");
            if (!SscoPosManager.getInstance().isUsed()) {
                oplLine.init(Mnemo.getText(81)).show(1);
                panel.clearLink(Mnemo.getInfo(74), 2);
            }
            MarshallEftPlugin eftPlugin = (MarshallEftPlugin) eftPluginManager.getPlugin(tnd[itm.tnd].eftPlugin);
        ...
        logger.info("tnd[itm.tnd].eftPlugin >" + tnd[itm.tnd].eftPlugin + "<");
        sts = eftPluginManager.doTransaction(tnd[itm.tnd].eftPlugin, itm, ctl, oplLine);
        if (sts != 0) {
        ...
        //if (ptr.type == 'L') {
        if ("LS".indexOf(ptr.type) >= 0) {
            CustomTender account = GdPsh.getInstance().getCustomTender(itm.tnd);
            ...
            ind = GdPsh.getInstance().redemptionGiftCard(itm, tra);
            if (ind > 0)  //INTEGRATION-PHILOSHOPIC-CGA#A END
                return ind;
        }
