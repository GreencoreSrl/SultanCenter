...
// PSH-ENH-001-AMZ#ADD -- entire module
//
// === === === == PhiloShopic integration == === === === 
// 2015 By Andrea Michele Zoia - Greencore for NCR Corp.
//
/**
 ...
 */
public class GdPsh extends Action {
	...
	private static boolean par_isEnabled = false;
	...
	private static final String LOYALTY = "LOYALTY_CARD";
    ...
    private static boolean par_isEnabledPXL = false;  // AMZ-2017-003-004#ADD
    private static boolean par_isEnabledSMASH = false; // AMZ-2017-003-004#ADD
    private static boolean par_isEnabledBarcode = false; // AMZ-2017-003-004#ADD
    private static boolean par_noAskPrice = false; // AMZ-2017-003-004#ADD
	...
	static void printBarcode(){
        ...
        if(!par_isEnabledBarcode){
			logger.info("EXIT printBarcode() - par_isEnabledBarcode: disabled");
            return;
        }
        if(uniqueTransactionId.length()==0){
			logger.info("EXIT printBarcode() - uniqueTransactionId.length(): " + uniqueTransactionId.length());
            return;
        }
        // AMZ-2017-003-FIX1#END
		logger.info("EXIT printBarcode() - call DevIo.tpmLabel(2, uniqueTransactionId);");
        DevIo.tpmLabel(2, uniqueTransactionId);
    }
    // AMZ-2017-003#END


	// PSH-ENH-001-AMZ#BEG -- minifont
	//static void minifont(boolean mf) {
	//	panel.dspArea[2].setFont(mf ? panel.font40 : panel.font20);
	//}
	// PSH-ENH-001-AMZ#END
	...
	static boolean isEnabled() {
		return par_isEnabled;
	}

    // AMZ-2017-003-004#BEG
    static boolean isEnabledSMASH() {
        return par_isEnabledSMASH;
    }
    static boolean isEnabledBarcode() {
        return par_isEnabledBarcode;
    }
    static boolean isEnabledPXL() {
        return par_isEnabledPXL;
    }
    static boolean isNoAskPrice() {
        return par_noAskPrice;
    }
	...
	static void readPregpar(String txt, int ind) throws Exception {
		...
		par_isEnabled = (Integer.parseInt(txt.substring(0, 2)) == 1);
		...
		readConfig();
		if (par_isEnabled) {
			System.out.println("PSHP:Enabled");
			System.out.println("PSHP:Primary Server endpoint " + par_httpPrimaryServer);
			System.out.println("PSHP:Secondary Server endpoint " + par_httpSecondaryServer);
			System.out.println("PSHP:Server timeout " + par_httpTimeoutSec);
			System.out.println("PSHP:Keyboard " + (par_kbdenabled ? "enabled" : "disabled"));
			System.out.println("PSHP:topup card prefix " + par_prefix);
			System.out.println("PSHP:topup item map on EAN " + par_prefixToEan);
			System.out.println("PSHP:Store number " + par_storeid);
            System.out.println("PSHP:Primary Server endpoint utility " + par_httpPrimaryServerUtility);
            System.out.println("PSHP:Secondary Server endpoint utility " + par_httpSecondaryServerUtility);
			System.out.println("PSHP:Primary Server endpoint tlog " + par_httpPrimaryServerTlog);
			System.out.println("PSHP:Secondary Server endpoint tlog " + par_httpSecondaryServerTlog);
            System.out.println("PSHP:Store number utility " + par_storeidUtility);
            System.out.println("PSHP:check Amount Gift card " + par_checkAmtGift);
            // AMZ-2017-003-004#BEG
            logger.debug("PSHP:SMASH enabled " + par_isEnabledSMASH);
            System.out.println("PSHP:SMASH enabled " + par_isEnabledSMASH);
            logger.debug("PSHP:Barcode printing enabled " + par_isEnabledBarcode);
            System.out.println("PSHP:Barcode printing enabled " + par_isEnabledBarcode);
            logger.debug("PSHP:PXL enabled " + par_isEnabledPXL);
            System.out.println("PSHP:PXL enabled " + par_isEnabledPXL);
            logger.debug("PSHP:no ask price to cashier " + par_noAskPrice);
            logger.debug("PSHP:utility without properties " + isEnableUtilityWithoutPr);
            System.out.println("PSHP:no ask price to cashier " + par_noAskPrice);
            // AMZ-2017-003-004#END
        }
	...
	static void readConfig() throws Exception {
		try {
			...
			// AMZ-2017-003-004#BEG
            try {
                par_isEnabledSMASH = Boolean.parseBoolean(prop.getProperty("isEnabled"));
            }catch(Exception e){
                throw new Exception("missing or malformed isEnabled parameter in file " + PROP_FILENAME);
            }
            try {
                par_isEnabledBarcode = Boolean.parseBoolean(prop.getProperty("barcodeEnabled"));
            }catch(Exception e){
            ...
            try {
				...
				if (GdPsh.isEnableUtilityWithoutPr()) {
					return ;
				}
			...
			try {
				par_isEnabledPXL = Boolean.parseBoolean(prop.getProperty("isEnabled", "false"));
			}catch(Exception e){
				throw new Exception("missing or malformed isEnabled parameter in file " + PROP_PXL_FILENAME);
			}
			try {
				par_noAskPrice = Boolean.parseBoolean(prop.getProperty("noAskPrice", "false"));
			}catch(Exception e){
				throw new Exception("missing or malformed noAskPrice parameter in file " + PROP_PXL_FILENAME);
			}
            // AMZ-2017-003-004#END
	...
	int action1(int spec) {
		...
		if (cus.number.trim().length() == 0) {
			logger.debug("ENTER action1_3");
			return 7;
		}
		logger.info("customer number: " + cus.number);
		...
		panel.clearLink(Mnemo.getDiag(MNEMO_DIAGS_BASE + 3).trim() + " " + cus.pnt, 0x81);
    ...
    static int insertMobile() {
        ...
        cus.mobile = newInput.pb;

        logger.info("mobile: " + cus.mobile);
    ...
    //PSH-ENH-20151120-CGA#A END
       ...
       */
	static int readSerial32(Itemdata plu) {
	...
	static void setCashierId(int posId, int cashierId) {
		if (!par_isEnabled) return;

        if (par_isEnabledSMASH) { // AMZ-2017-003-004#ADD
        ...
        if (par_isEnabledPXL) { // AMZ-2017-003-004#ADD
	 ...
	 * @return 0 if ok, >0 error code in nmemo
	 */
	static int payGiftCard(Itemdata itm, Transact tra) {
		...
		// itm.amt l'ammontare
		// tra.bal quanto resta da pagare per la transazione
		if (itm.amt > tra.bal) {
	 ...
	 * @return 0 if ok, >0 error code in nmemo
	 */
	static int cancelRedemption(Itemdata itm) {
	...
	private static int srv_payGiftCard(Itemdata itm) {
		String ret[] = srvDoTransaction(DEBIT, GIFTCARD, itm.gCardSerial, itm.amt);
	...
	private static int srv_totalGiftCardBalance(Itemdata itm) {
		String[] reply = srvGetAccountBalance(GIFTCARD, itm.gCardSerial);
	...
	private static int srv_cancelRedemption(Itemdata itm) {
		String ret[] = srvCancelTransaction(DEBIT, GIFTCARD, cus.number, itm.prpnt, itm.redemptionTransaction);
		if (!REPLY_OK_VALUE.equals(ret[0])) {
	...
	private static int srv_cancelPayGiftCard(Itemdata itm) {
		String ret[] = srvCancelTransaction(DEBIT, GIFTCARD, itm.gCardSerial, itm.amt, itm.gCardTransaction);
		if (!REPLY_OK_VALUE.equals(ret[0])) {
			return MNEMO_ERROR_BASE + 0;
		}
		return 0; // ok
	 ...
	 */
	static int cancelGiftCard(Itemdata itm) {
		if (itm.gCardTopup) {
	...
	static boolean isGiftCard(Itemdata itm) {
		return (itm.gCard == '1') && par_isEnabled;
	}

    //PSH-ENH-20151120-CGA#A BEG
	static boolean isUtility(Itemdata itm) {
        logger.debug("ENTER isUtility");
        logger.info("flag: " + itm.gCard);
        logger.debug("EXIT return: " + ((itm.gCard == '2') && par_isEnabled));

		return (itm.gCard == '2') && par_isEnabled;
	}
    ...
    private static String[] srvDoPrepayXLTransactionActivation(boolean online) {
        ...
        try {
            ...
            if (online) {
                ...
                message = ppController.beginTransaction(itm.utilityName, String.valueOf(itm.price), cus.mobile, uniqueTransactionId); // AMZ-2017-003#ADD
            } else {
                // message = ppController.beginTransaction(itm.utilityName);// AMZ-2017-003#DEL
                message = ppController.beginTransaction(itm.utilityName, uniqueTransactionId);// AMZ-2017-003#ADD
            }
            ...
            if (Integer.parseInt(reply[0]) <= 0) {
                ...
                for (int time = 1; time <= SRV_RETRY; time++) {
                    ...
                    if (online) {
                        message = ppController.processTransaction(itm.utilityName, String.valueOf(itm.price), cus.mobile, transactionId);
                    } else {
    ...
    static int cancelBuyUtility(Itemdata itm) {
        logger.debug("ENTER cancelBuyUtility");
        logger.info("item code: " + itm.number.trim());

        if (!par_isEnabled) {
            logger.debug("EXIT cancelBuyUtility - philoshopic disabled, return error message");
        ...
        // AMZ-2017-003-004#BEG
        if (!par_isEnabledPXL) {
            logger.debug("PXL controller disabled. exiting");
            return 7; // unavailable
        }
        //AMZ-2017-003-004#END
	...
	private static int srv_customerCheck(Customer cus) {
		...
		String[] reply = srvGetAccountBalance(LOYALTY, cus.number);
	...
	private static int dmy_customerCheck(Customer cus) {
		cus.pnt = (int)Double.parseDouble("100.00");
	...
	static void customerUpdate(Customer cus, Transact tra) {
		...
		if (ret != 0) {
            if(cus.number!=null) { // AMZ-2017-003-002#ADD
                Itmdc.IDC_write('Z', sc_value(tra.spf1), 0, cus.number, SYNCERROR_POINTS, tra.pnt - cus.pnt);
            // AMZ-2017-003-002#BEG
            }else{
                Itmdc.IDC_write('Z', sc_value(tra.spf1), 0, "0000000000000", SYNCERROR_POINTS, tra.pnt - cus.pnt);
            }
	...
	private static int srv_customerUpdate(Customer cus, Transact tra) {
		String ret[] = srvDoTransaction(CREDIT, LOYALTY, cus.number, tra.pnt + tra.prpnt - cus.pnt, tra.amt, tra.gctnd);
		if (!REPLY_OK_VALUE.equals(ret[0])) {
			return MNEMO_ERROR_BASE + 0;
		}
		try {
	...
	private static int srv_pointsRedemption(Itemdata itm) {
		String ret[] = srvDoTransaction(DEBIT, LOYALTY, cus.number, -itm.prpnt);
		if (!REPLY_OK_VALUE.equals(ret[0])) {
			return MNEMO_ERROR_BASE + 0;
		}
		try {
	...
	static int customerCheck(Customer cus) {
		...
		if (SscoPosManager.getInstance().isUsed()) {
			...
			((LoyaltyRequestProcessor)(SscoPosManager.getInstance().getProcessor())).setLoyaltyInfo(new SscoCustomer(cus.number, "", ""));
        ...
        if(cus.selfSellEANList.length()>0) {
			tra.number = cus.number; //CGA-2017-003-006#A
            String eans[] = cus.selfSellEANList.split(",");
            int sts;
            ...
            for(String ean : eans){
                ...
                if (event.spc == 0) {
                    itm.price = itm.prpov;
                    sts = group[5].action7(input.num = 0);
                }
                */
                if (sts != 0) {
                    continue;
	...
	private static String[] srvGetAccountBalance(String accountType, String accountNo) {
		System.out.println("PSHP:srvGetAccountBalance : " + accountType + ", " + accountNo);
		String reply[] = { "999" };
		for (int time = 1; time <= SRV_RETRY; time++) {
			String message = controller.getAccountBalance(accountType, accountNo, null);
			System.out.println("PSHP:Server reply : " + message);
			reply = message.split(";");
			if (REPLY_OK_VALUE.equals(reply[0])) {
				return reply;
			}
	...
	private static String[] srvDoTransactionActivation(String transactionType, String accountType, String accountNo,
        ...
        if(accountNo!=null) {   // AMZ-2017-003-002#ADD
            System.out.println("PSHP:srvDoTransactionActivation : " + transactionType + ", " + accountType + ", "
                    + mask(accountNo) + ", " + value + ", " + amt + ", " + amt2 + ", uniqueTransactionId: " + uniqueTransactionId);
            // AMZ-2017-003-002#BEG
        } else {
            System.out.println("PSHP:srvDoTransactionActivation : " + transactionType + ", " + accountType + ", "
                    + "no customer" + ", " + value + ", " + amt + ", " + amt2 + ", uniqueTransactionId: " + uniqueTransactionId);
        }
        // AMZ-2017-003-002#END
        ...
        //PSH-ENH-20151120-CGA#A BEG
       if (amt < 0 && amt2 < 0 && transactionType.equals("DEBIT") && accountType.equals(GIFTCARD)) {
            String[] replyAmt = srvGetAccountBalance(GIFTCARD, itm.gCardSerial);
            long amtGift = 0;
		...
		if (amt >= 0) {
			valueField = new StringBuilder(valueField).append(";").append(moneyForServer(GIFTCARD, amt)).append(";").append(moneyForServer(GIFTCARD, amt2)).toString();
		}
		if (transactionType.trim().length() > 0) {
            ...
            message = controller.beginTransaction(transactionType, accountType, accountNo, valueField, uniqueTransactionId);    // AMZ-2017-003#ADD
		} else {
			// message = controller.beginActivation(accountType, accountNo, valueField); // AMZ-2017-003#DEL
			logger.info("Call beginActivation. accountType: " + accountType +" accountNo: " +accountNo +" valueField: " + valueField + " uniqueTransactionId: " + uniqueTransactionId);
            message = controller.beginActivation(accountType, accountNo, valueField, uniqueTransactionId); // AMZ-2017-003#ADD
		}
		System.out.println("PSHP:Server reply : " + message);
		...
		try {
			if (Integer.parseInt(reply[0]) <= 0) {
				...
				System.out.println("PSHP:Process " + transactionType + ", " + accountType + ", " + mask(accountNo) + ", "
				...
				for (int time = 1; time <= SRV_RETRY; time++) {
					...
					// ERROR HANDLING
					System.out.println("PSHP:Error for transaction : " + transactionId);
					if (Integer.valueOf(reply[0]) < 100) {
						break; // non-recoverable error such as account out of credit.
					}
				}
			} else {
				System.out.println("PSHP:Could not get transactionID. Exit.");
			}
		} catch (NumberFormatException e) {
	...
	private static String[] srvCancelTransactionActivation(String transactionType, String accountType, String accountNo,
			long value, String transactionId) {
		System.out.println("PSHP:srvCancelTransactionActivation : " + transactionType + ", " + accountType + ", "
	...
	public static void sendDataCollect(){
		...
		if (tra.mode == M_GROSS) {
			...
			while (tlogResult.indexOf(TLOG_OK)<0 && retry > 0) {
				logger.info("send datacollect : logSTORE(" + uniqueTransactionId + ", dc_length: " + dataCollect.length() + " ) ");
				logger.debug("send datacollect : logSTORE(" + uniqueTransactionId + ",\n" + dataCollect + " ) ");
				tlogResult = logController.logSTORE(uniqueTransactionId, dataCollect);
				retry--;
