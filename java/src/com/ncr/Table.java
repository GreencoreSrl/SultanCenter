...
abstract class Table extends Struc {
	...
	static BinIo lRCD = new BinIo("RCD", 4, 30, !ECommerceManager.getInstance().isEnabled());
	//ECOMMERCE-SSAM#C BEG
	...
	static void posWrite(int sc, int trans, long total) {
		...
		if (sc == 1) {
			if (lPOS.trans < 1)
				lPOS.trans++;
			ctl.tran = lPOS.trans;
			ctl.uniqueId = "20" + ctl.date + editNum(ctl.sto_nbr, 3) + editNum(ctl.reg_nbr, 3) + editNum(ctl.tran, 4);  //INTEGRATION-PHILOSHOPIC-CGA#A
		}
		if (sc == 2)
			ctl.zero = lPOS.trans;
		lPOS.rewrite(sc);
