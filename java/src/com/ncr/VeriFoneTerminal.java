...
public class VeriFoneTerminal extends Action {
    private static final Logger logger = Logger.getLogger(VeriFoneTerminal.class);
    ...
    static final int ERR_TRANSBASE = 104;
    static final int ERR_TIMEOUTTRANSACTION = 71;
    static final int ERR_NOTCONFIGURED = 72;
    static final int ERR_RESPONSE = 73;

    /*Constant to identify voucher type*/
    public static final String NEW_RECEIPT_TYPE = "S_PLURCO.DAT";
    public static final String SAME_RECEIPT_TYPE = "S_PLURCP.DAT";
    ...
    private static boolean enableCustomerCopy = false;
    private static boolean enableMerchantCopy = false;
    private static HashMap receiptDatas = new HashMap();
    private String authorizationCode = "";
    private String cardType = "";
    private static boolean risSettle = false;


    public VeriFoneTerminal() {
    }
    ...
    public static void loadVeriRow(String row) {
        enableMerchantCopy = row.substring(0, 1).equals("1");
        enableCustomerCopy = row.substring(1, 2).equals("1");
        settleEnabled = row.substring(2, 3).equals("1");
        testEnvironment  = row.substring(3, 4).equals("1");
    }
    ...
    public int doTransaction(long amt, String traNum) {
        ...
        if (!initialize) return ERR_NOTCONFIGURED;
    ...
    int action0(int spec) {
        ...
        try {
            ...
            verifone.printVouchers(VeriFoneTerminal.NEW_RECEIPT_TYPE);
            verifone.printVouchers(VeriFoneTerminal.SAME_RECEIPT_TYPE);

            //GdRegis.prt_trailer(1);

            logger.info("1 itm.tnd: " + itm.tnd);
            itm.tnd = ExtResume.getExtTenderNumber(verifone.getCardType(), itm.tnd);
    ...
    int action1(int spec) {
        if (PluginHandler.getInstance().isVerifonePluginEnabled()) {
    ...
    public void printVouchers(String type) {
        ...
        if (type.equals(SAME_RECEIPT_TYPE) && receiptDatas.containsKey(type)) {
            Vector receiptData = (Vector) receiptDatas.get(SAME_RECEIPT_TYPE);
            if (receiptData != null && receiptData.size() > 0 && enableCustomerCopy) {
                for (int index = 0; index < receiptData.size(); ++index) {
                    logger.debug("Same Receipt Line [" + index + "] : "+ (String) receiptData.get(index));
                    Struc.prtLine.init((String) receiptData.get(index)).book(3);
                }
                Struc.prtLine.init(' ').book(3);
            }
        } else if (type.equals(NEW_RECEIPT_TYPE) && receiptDatas.containsKey(type)) {
            Vector receiptData = (Vector) receiptDatas.get(NEW_RECEIPT_TYPE);
            if (receiptData != null && receiptData.size() > 0 && enableMerchantCopy) {
                for (int index = 0; index < receiptData.size(); ++index) {
                    logger.debug("New Receipt Line [" + index + "] : "+ (String) receiptData.get(index));
                    Struc.prtLine.init((String) receiptData.get(index)).type(2);
                }
